...
A facility organization is the facility-scoped form of the [organization](../access-governance/organization.mdx) primitive (`FacilityOrganization`). It carries a name and description, a place in the facility's internal tree, a type, and a list of member users — each with the role they hold.
...
`dept` and `team` carry the everyday structure of a hospital; `role` provides facility-level user groups (the facility-scoped counterpart of an instance [role organization](../access-governance/organization.mdx)).
...
- **Facility** — every facility organization belongs to exactly one [facility](../facility/facility.mdx) and shares its lifetime.
...
- **Locations** — physical [locations](../facility/location.mdx) such as wards and beds are modelled separately; a facility organization groups people and access, not rooms (though an encounter's current location also contributes to who can reach it).
...
- Concept: [Organization](../access-governance/organization.mdx) · [Facility](../facility/facility.mdx) · [Encounter](../clinical/encounter.mdx) · [Location](../facility/location.mdx)
