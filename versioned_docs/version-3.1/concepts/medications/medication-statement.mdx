...
If your facility is actively prescribing a drug, that belongs in a [medication request](./medication-request.mdx). Reach for a statement when the source of truth lives outside that order — a home medication list, a drug started elsewhere, or a patient's own account of what they take.
...
- **Medication request** — the active-order counterpart. Use a [medication request](./medication-request.mdx) when your facility is the one prescribing; use a statement to capture everything else.

## Permissions

Access to medication data is governed by facility-scoped permissions. The permissions below come from Care's medication permission set; they cover the pharmacist capability and the dispense actions that share the medication workflow.

| Permission | Description | System Roles |
| --- | --- | --- |
| Pharmacist in Care | Pharmacists in Care have access to all medication requests and can create dispenses for patients | Facility Admin, Admin, Pharmacist |
| Medication Dispense Read | Users can read medication dispenses | Facility Admin, Admin, Staff, Doctor, Nurse, Pharmacist |
| Write Medication Dispense | Users can write medication dispenses | Facility Admin, Admin, Staff, Doctor, Nurse, Pharmacist |
...
- Concept: [Medication Request](./medication-request.mdx)
