...
Allergy and intolerance records have no permission file of their own — as patient clinical data, they are governed by the **patient** and **encounter** permissions a user holds in the relevant facility. Recording an allergy is gated by write access to the patient; reading and editing it fall back to the patient's clinical-data or the encounter's read/write permissions.
...
| `can_read_encounter` | Read an encounter and its clinical data — used to reach allergies when patient-level clinical access is absent | Admin, Doctor, Nurse, Staff, Facility Admin |
| `can_write_encounter` | Update an allergy record (the update path checks write access to its encounter) | Admin, Doctor, Nurse, Staff, Facility Admin |
| `can_list_patients` | List patients in scope (entry point to reach a patient's record) | Staff, Doctor, Nurse, Administrator, Admin, Facility Admin, Volunteer, Pharmacist |
