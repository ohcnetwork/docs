...
A **role** is an arbitrary set of permissions — `Doctor`, `Doctor (Read Only)`, `Doctor (Scheduler)` — nothing more than a named bundle, never a job title. A **role organization** is a group of users. They are independent: you grant a role to a user *within* an organization. See [Roles & permissions](../access/access-control.mdx).
:::
...
- Concept: [Roles & permissions](../access/access-control.mdx) · [Facility Organization](./facility-organization.mdx) · [Facility](./facility.mdx)
