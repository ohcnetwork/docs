...
`UserResourceFavorites` extends [`EMRBaseModel`](../foundation/base-model.mdx), the shared Care EMR base providing `external_id`, the audit fields `created_by` / `updated_by` / `created_date` / `modified_date`, and soft-delete semantics.
...
In practice, the tuple `(user, resource_type, facility, favorite_list)` uniquely addresses a row. The cache keys and the `get_or_create` in `add_favorite` are both built from this combination.

### `resource_type` values — `FavoriteResourceChoices`

`resource_type` is constrained at the API layer to the string values of `FavoriteResourceChoices` (`care/emr/resources/favorites/spec.py`). Each value is wired to a viewset through that viewset's `FAVORITE_RESOURCE` attribute.
...
Favorites do **not** use the `EMRResource` Create/Update/List/Retrieve pattern. The "spec" surface consists of:

| Symbol | Kind | Role |
| --- | --- | --- |
| `FavoriteResourceChoices` | `str, Enum` (`spec.py`) | The allowed `resource_type` values (table above). A viewset selects one via `FAVORITE_RESOURCE = FavoriteResourceChoices.<x>.value` |
...
There is no read schema for the favorited objects themselves. Favorited resources come back through the **owning resource's own list endpoint** (for example, the questionnaire list serializer), filtered and ordered by `FavoritesFilter`. The `favorite_lists` action returns only `{"lists": [<list name>, ...]}`.
...
Two module-level helpers build the cache keys, encoding a missing facility as `-`:
...
`save()` sets `refresh_list = not self.pk` (so only when creating a new row), calls `super().save()`, then `refresh_cache(refresh_list=refresh_list)`. The effects:

1. Creating a new list refreshes both the lists cache and the list-object cache.
2. Updating an existing list refreshes only the list-object cache, since the set of list names is unchanged.

Integrators reading favorites through the cache should treat these keys as platform-maintained. Write through the model or endpoints so the cache stays consistent.

## API integration notes

Favorites are exposed through the **owning resource's viewset** via `EMRFavoritesMixin`, not through a standalone favorites endpoint. A viewset opts in by mixing in `EMRFavoritesMixin`, setting `FAVORITE_RESOURCE`, and adding `FavoritesFilter` to `filter_backends`:
...
| `favorite_lists` | `GET {resource}/favorite_lists/` | — | Returns `{"lists": [<name>, ...]}` (cache-first, falls back to DB). Resolves `facility` from `facility_external_id` kwarg or `?facility=` query param |
...
- `resource_type` is never client-supplied; each viewset fixes it via `FAVORITE_RESOURCE`. The client only ever sends `favorite_list`.
- `facility` is derived server-side via `retrieve_facility_obj(obj)` (defaulting to `obj.facility`), so favorites inherit the favorited object's facility scope.
...
- The list is capped at `settings.MAX_FAVORITES_PER_LIST` (env `MAX_FAVORITES_PER_LIST`, default `50`). On `add_favorite`, the oldest entries beyond the cap are dropped.
