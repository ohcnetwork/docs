...
| Field definitions | `ReportUpload` fields |
...
A template stores report markup, render format, context binding, and generator options. The API constrains `status`, `default_format`, the slug, and the compatibility of `template_type`, `context`, and `options`.
