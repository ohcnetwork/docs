...
> `ReportUpload` has no `CreateSpec`/`UpdateSpec` in this module. Uploads come from the file-upload flow ([File Upload](./file-upload.mdx)), and the report metadata and `internal_name` are platform-maintained. See [save behaviour](#methods--save-behaviour).
