...
`TagConfig` is self-referential — tags form a tree, and denormalized caches are rebuilt on insert to avoid recursive joins (the same pattern as [`Organization`](../access-governance/organization.mdx)). These fields are platform-maintained and never writable through the create/update specs.
...
- Create — `TagConfigWriteSpec`. Sets `resource`, the scope FKs, and `parent`. `resource` must match a [`TagResource`](#tagresource-values) value.
...
- Reference: [Organization](../access-governance/organization.mdx) — tags reuse the same tree-cache pattern and can be org-scoped
