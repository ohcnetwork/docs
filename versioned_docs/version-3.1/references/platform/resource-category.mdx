...
`ResourceCategory` is the **storage** layer (Django model). The **API/implementation** layer comes from the Pydantic resource specs in `care/emr/resources/resource_category/`, which add the `resource_type` enum, the structured shape of the `*_monetary_components` JSON fields (via `MonetaryComponent`), slug validation, and the read/write schemas. Several model fields are opaque `JSONField`s whose real structure lives only in the specs — see [Resource specs (API schema)](#resource-specs-api-schema).
...
`ResourceCategory` extends [`SlugBaseModel`](../foundation/base-model.mdx) (`FACILITY_SCOPED = True`), the facility-scoped slug variant of the base. It adds `slug` handling on top of the EMR base fields: `external_id`, audit fields, soft-delete, and `history`/`meta` JSON.
...
| `slug` | `CharField(255)` | yes | Facility-scoped encoded slug `f-<facility_external_id>-<slug_value>`. Clients send the unencoded `slug_value` (validated `SlugType`); the model encodes it via `calculate_slug()` |
| `description` | `TextField` | no | Nullable, blank-able (`description: str \| None = None`) |

`Meta` declares a composite index on `(slug, facility)` for fast slug lookups within a facility.
...
`set_organization_cache()` and `get_parent_json()` populate these denormalized fields so the ancestor chain reads without recursive joins. They are platform-maintained — clients never write them.
...
The shape of `cached_parent_json` (and of the `parent` field in `ResourceCategoryReadSpec`), as built by `get_parent_json()`:
...
> The category's monetary fields are stored and validated per-element as `MonetaryComponent`. The stricter collection validators (`MonetaryComponents`/`MonetaryComponentsWithoutBase`: single base, no duplicate codes, tax-sum balance) live with the charge-item resources, not the category list itself.

## Methods & save behaviour

`ResourceCategory` overrides `save()` and maintains tree and charge caches through several methods plus a Celery task.
...
Returns (and lazily rebuilds) `cached_parent_json`. If a cached snapshot exists and has not passed its `cache_expiry`, it is returned as-is. Otherwise the method walks the parent chain, builds a fresh nested snapshot with a new expiry (`now + cache_expiry_days`), persists it via `save(update_fields=["cached_parent_json"])`, and returns it. Root nodes (no `parent_id`) return `{}`.
...
- Categories are facility-scoped; the `(slug, facility)` index and `SlugBaseModel` slug encoding (`f-<facility>-<slug>`) keep slugs unique per facility. Clients send `slug_value` (5–50, URL-safe); the server encodes it and `slug_config` decodes it.
- `parent` is written by **slug** (`ResourceCategoryWriteSpec.parent` → `ResourceCategory.objects.get(slug=...)`), not by id; on read it is the nested `get_parent_json()` snapshot.
- `parent_cache`, `level_cache`, `root_org`, `has_children`, `cached_parent_json`, and `calculated_monetary_components` are platform-maintained — do not set them directly. Write `parent` and `configured_monetary_components`, and let `save()` and the Celery task derive the rest.
...
- Monetary-component fields serialize only for `resource_type == "charge_item_definition"`; in merges, component identity relies on the nested `code.system` + `code.code` pair.
