...
Technical reference for the `ValueSet` module in Care EMR. A value set is a FHIR-aligned composition of coded concepts drawn from one or more code systems; it constrains the options a [questionnaire](../forms/questionnaire.mdx) can offer for a coded answer.

The Django model is the **storage layer** — `compose` is an opaque `JSONField`. The real structure (include/exclude clauses, filters, status enum, validation, and the read/write API schemas) lives in the Pydantic **resource specs**. This page documents both.
...
`ValueSet` and `UserValueSetPreference` extend [`EMRBaseModel`](../foundation/base-model.mdx) (the shared Care EMR base with `external_id`, audit fields, `history`/`meta` JSON, and soft-delete semantics). `RecentViewsManager` is a plain Python class backed by Redis, not a Django model.

## `ValueSet` fields

A value set is **not** a flat list of codes. It stores a `compose` object that describes *rules* — include/exclude clauses against code systems. The member codes themselves are resolved in real time against terminology servers when the value set is searched or a code is looked up.
...
`compose` holds a `ValueSetCompose` structure (`care.emr.resources.common.valueset`). All nested models set `extra="forbid"`, so unknown keys are rejected. Each clause targets a `system` (code system URI) and may select concepts directly or via filters.
...
Not a database model — a classmethod-only helper that tracks a user's recently viewed codes in a Redis list (one list per `cache_key`). It surfaces recently used codes alongside favorites in pickers.
...
`MAX_RECENT_VIEW` defaults to `20` (overridable via `settings.MAX_RECENT_VIEW_FOR_VALUESET`). Entries are JSON-encoded, and malformed entries are skipped on read and remove.
...
Because membership resolves live, editing `compose` immediately changes which codes the value set yields — there is no expansion or cache to rebuild.
...
- Value sets store composition *rules*, not expanded code lists. Members are queried in real time from terminology servers, so results can shift as upstream systems change.
