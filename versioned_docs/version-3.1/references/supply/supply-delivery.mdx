...
A supply delivery records product moving into a destination location — either from another location or from an external supplier. Deliveries and dispensing together drive facility stock: `supply delivered − supply dispensed = current stock`. Each delivery belongs to a `DeliveryOrder`, the shipment grouping usually created from a [supply request](../supply/supply-request.mdx).
