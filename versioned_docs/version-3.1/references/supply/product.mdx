...
| Field definitions | `Product` fields |
...
| --- | --- | --- | --- | --- |
| `batch` | `JSONField` (nullable) | no | `dict` / `None` | Spec shape: [`ProductBatch`](#productbatch-shape) `{ lot_number: str \| None }`. Use only the optional `lot_number` key. |
