...
`Product` is the STORAGE layer (Django model). The API/implementation layer lives in the Pydantic **resource specs**, which define the enums, the structured shape of the model's opaque `JSONField`s, the validation rules, and the read/write schemas. Several model fields are opaque `JSONField`s whose real structure surfaces only in the specs — see [Resource specs (API schema)](#resource-specs-api-schema).
...
`Product` extends [`EMRBaseModel`](../foundation/base-model.mdx) (the shared Care EMR base providing `external_id`, audit fields, and soft-delete semantics).

A `Product` carries only the data unique to a particular batch — lot number, expiry, purchase price. All catalogue-level detail (name, codes, dosage form) lives on the linked [`ProductKnowledge`](../supply/product-knowledge.mdx); a `Product` instantiates that catalogue entry at a facility.
...
| Field | Type | Notes |
| --- | --- | --- |
| `facility` | `FK → facility.Facility` (`PROTECT`) | Facility the product belongs to. Required at storage; not a spec field — the viewset sets it from the URL context rather than the request body. |
| `product_knowledge` | `FK → emr.ProductKnowledge` (`PROTECT`) | Catalogue definition this product instantiates. On write the spec accepts a `slug` string and resolves it; on read it serializes to the nested [`ProductKnowledge`](../supply/product-knowledge.mdx) read object. |
...
- **Extension validation**: on write, `extensions` passes through `ExtensionValidator.validate_extensions` against the JSON schemas registered for `ExtensionResource.product`; keys with no registered handler are silently dropped (current behaviour, marked TODO to become an error).
...
- On write, send `product_knowledge` and `charge_item_definition` as **slugs**; on read they return as full nested objects.
- `charge_item_definition` links the product to billing, so charge items are created automatically when the product is billed.
- `batch` is a structured object (`{ lot_number }`), not a free-form dict, even though the model stores it as a `JSONField`.
