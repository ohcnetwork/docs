...
| Field definitions | `RoleModel` fields |
...
| `contexts` | `ArrayField[CharField(24)]` | yes | `[]` | The boundary types this role applies to. Each element must be one of the **`RoleContext`** enum values (see [RoleContext values](#rolecontext-values)) |
