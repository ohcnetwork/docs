...
`FacilityFlag` is a server-side feature-gating primitive: it associates a registered flag name with one `Facility` so behaviour can be toggled per facility. It is **not** a FHIR resource and has **no dedicated Pydantic resource spec**; there is no `FacilityFlag` API endpoint, serializer, or `...CreateSpec`/`...ReadSpec`. This page documents the model, inherited flag helpers, cache behaviour, and the derived facility read payload. Flags reach API clients only as a read-only array on the Facility resource (see [Resource specs](#resource-specs-api-schema)).
...
| Field definitions | `FacilityFlag` fields |
...
`flag` values themselves are not enumerated in source; they are registered dynamically (`FlagRegistry.register(FlagType.FACILITY, "<name>")`). A name not present in the `FACILITY` registry raises `FlagNotFoundError` on save.
