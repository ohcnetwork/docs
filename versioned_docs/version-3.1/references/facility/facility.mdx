...
A `Facility` is a physical or virtual care site, such as a hospital, clinic, lab, or telemedicine endpoint, and the root of a deployment's administrative hierarchy.
...
| Field definitions | `Facility` fields |
| Allowed values | Choices & enums |
| Structured payloads | Nested JSON shapes |
...
| `print_templates` | `JSONField` | `default=list`. Write as `list[PrintTemplate]` (see [PrintTemplate](#printtemplate-json-shape)). Accepted typed on create; returned as `list[dict]` on retrieve. |
