...
The Django model is the **storage layer**, and it types several fields only loosely (`CharField` for status enums, `JSONField` for `contact`/`metadata`). The **Pydantic resource specs** under `care/emr/resources/device/` form the API/implementation layer: they define the enums those `CharField`s are constrained to, the structured shape of the JSON fields, the validation rules, the read/write schemas, and the server-maintained side effects.
...
`device` uses `PROTECT`, so a `Device` with service-history rows cannot be hard-deleted while those rows exist. `edit_history` is an append-only audit list maintained server-side on every update (see [Methods & save behaviour](#deviceservicehistory-edit-history)).
...
- `status` and `availability_status` must each be one of their enum values, or the request is rejected.
...
- `care_type` cannot change on update (`DeviceUpdateSpec` does not expose it).
...
This cache lets devices be filtered by organization hierarchy without traversing joins. Treat it as platform-maintained, and use `managing_organization` / `facility` as the writable inputs.
...
On `perform_update`, the viewset snapshots the **current** DB row and appends `{serviced_on, note, updated_by}` to `edit_history` before saving the new values, making `edit_history` a server-maintained, append-only trail of previous states. Updates are refused once it holds 50 entries.

## API integration notes

- `facility` comes from the URL (`facility_external_id`) on create, not from the body.
- `current_location`, `current_encounter`, and `managing_organization` are **not** writable through the device create/update body. Dedicated detail actions mutate them, and each one closes the open history row (stamps `end`) and opens a new one:
  - `POST associate_encounter` — `{ encounter: uuid | null }`; closes the open `DeviceEncounterHistory`, sets `current_encounter`, and opens a new history row (which it returns). `null` detaches.
...
- `facility_organization_cache` is platform-maintained — do not set it directly. The list endpoint scopes results to devices whose cache overlaps the requesting user's facility-organization memberships (or, when `?location=` is supplied, by location permission / `parent_cache` with `?include_children=`).
