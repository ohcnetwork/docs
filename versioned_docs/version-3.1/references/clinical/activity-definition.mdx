...
An Activity Definition is a FHIR-aligned, facility-scoped **template** for clinical activities. It holds the defaults needed to instantiate a downstream resource (currently a [Service Request](./service-request.mdx)) when the definition is "applied" against an encounter, merging those stored defaults with the encounter context.
...
Each field below lists its **storage type** (Django model) and, where the spec constrains it, the **API shape/values** the Pydantic specs enforce.
...
Stored as `JSONField`. The spec binds each to a value set via `ValueSetBoundCoding[<slug>]`, which validates the submitted code against that value set. Each coding is shaped `{ system?: str, version?: str, code: str (required), display?: str }` (`extra="forbid"`).
...
Stored as integer-ID arrays (`ArrayField(IntegerField, default=list)`) holding primary keys, not Django FK relations. The specs accept and return them by **external slug or UUID** rather than raw integer PKs:
...
- `version` / `latest` model an append-only version chain: clients select the active definition via `latest`, while older versions stay referenceable.

## API integration notes

- Payload field names follow the Pydantic specs and differ from the Django model: write `slug` as `slug_value`; reference `specimen_requirements` / `observation_result_requirements` / `charge_item_definitions` / `category` by **external slug** (`ExtendedSlugType`, `f-`/`i-` prefixed), and `locations` / `healthcare_service` by **UUID**.
...
- The "apply" operation instantiates the target `kind` (currently only `service_request`) by merging stored defaults with the encounter, and instantiates any linked `charge_item_definitions` at the same time.
