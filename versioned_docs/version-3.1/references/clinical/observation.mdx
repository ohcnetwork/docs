...
An observation is a single named, coded data point recorded about a subject, usually a patient, such as a blood pressure reading, a temperature, or a coded answer captured through a questionnaire. The `main_code` says *what* was observed, typically bound to LOINC, and `value` holds the recorded result.
...
| Field definitions | `Observation` fields |
| Allowed values | Enum & value-set reference |
| Structured payloads | Nested JSON shapes (from the spec) |
