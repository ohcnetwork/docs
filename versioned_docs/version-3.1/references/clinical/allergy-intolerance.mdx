...
The Django model is the **storage layer**. Several of its fields are opaque `JSONField`s whose real structure lives only in the Pydantic **resource specs** (the API/implementation layer). The specs define the enums, the JSON-field shapes, validation, the bound value set, and the read/write API schemas. This page documents both layers.
...
On write, the `code` field binds to this value set via `ValueSetBoundCoding[...slug]`, which validates the submitted `Coding` against the value set (`validate_valueset`). The compose includes SNOMED CT concepts that are `is-a` descendants of: `105590001`, `418038007`, `267425008`, `29736007`, `340519003`, `190753003`, `413427002`, `716186003`. The value set is also registered as a system (`register_as_system()`).

## Resource specs (API schema)

Every spec extends `EMRResource` (`care/emr/resources/base.py`), which provides `serialize` (DB object → pydantic, via `model_construct`) and `de_serialize` (pydantic → DB object), plus the `perform_extra_serialization` / `perform_extra_deserialization` hooks. `BaseAllergyIntoleranceSpec` sets `__model__ = AllergyIntolerance` and `__exclude__ = ["patient", "encounter"]`; these two fields are not auto-mapped and are handled by the hooks instead. The base also carries `meta` on every spec.
...
- **Create vs update surface differs.** `category`, `code`, `onset`, and `recorded_date` are write-only on **create** (`AllergyIntoleranceWriteSpec`) and are absent from `AllergyIntoleranceUpdateSpec`, so they cannot be changed after creation through the update path.
- **Read uses relaxed types.** `AllergyIntoleranceReadSpec` types the coded fields as plain `str` and `code` as `Coding`. It skips value-set re-validation on read for performance, relying on `model_construct`.
...
- `serialize` / `de_serialize` are inherited from `EMRResource`. `de_serialize` maps only DB fields that are not in `__exclude__` and not `id`/`external_id`, then calls `perform_extra_deserialization` for the encounter/patient wiring described above.
...
- Exposed through Care's REST API and aligned to the FHIR `AllergyIntolerance` resource. API field names follow the spec classes above — for example, `encounter` is sent and returned as a UUID `external_id`, not a numeric FK.
...
- The coded status fields (`clinical_status`, `verification_status`, `category`, `criticality`) accept only the enum values listed above; invalid values are rejected on write.
