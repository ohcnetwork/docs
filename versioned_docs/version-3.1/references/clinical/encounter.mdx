...
Technical reference for the `Encounter` module in Care EMR. For the plain-language view, read the [Encounter](../../concepts/clinical/encounter.mdx) concept.

An `Encounter` records a single interaction between a patient and a facility — a visit, admission, or consultation. You create one when a patient presents, drive it through its status lifecycle, and read it back to render the clinical timeline.
...
| Field | Type | Notes |
| --- | --- | --- |
| `facility_organization_cache` | `ArrayField[int]` | Platform-maintained cache of facility-[organization](../access-governance/organization.mdx) IDs (incl. parent chains). Rebuilt by `sync_organization_cache()` |
...
| `EncounterListSpec` | read · list | Adds serialized `patient` ([`PatientListSpec`](../clinical/patient)), `facility` (`FacilityBareMinimumSpec`), `status_history`, `encounter_class_history`, `created_date`, `modified_date`, `tags`, `current_location` (`FacilityLocationMinimalListSpec \| None`), `care_team` (`[{ member, role }]`) |
| `EncounterRetrieveSpec` | read · detail | Extends list + `EncounterPermissionsMixin`. Adds `appointment` (`TokenBookingReadSpec`), `created_by`/`updated_by` ([`UserSpec`](../access-governance/user.mdx)), `organizations` (`FacilityOrganizationReadSpec[]`), `location_history` (`FacilityLocationEncounter[]`, newest first), `extensions`; patient via `PatientRetrieveSpec` |
...
Associates an encounter with a facility-scoped [organization](../access-governance/organization.mdx) to drive organization-based access control on encounters.
