...
| `ConditionReadSpec` | read · detail/list | `clinical_status`, `verification_status`, `category`, `severity` (all plain `str`), `code` (plain `Coding`), `encounter` (UUID4), `onset`, `abatement`, `created_by?`, `updated_by?`, `note?`, `created_date`, `modified_date`. |
