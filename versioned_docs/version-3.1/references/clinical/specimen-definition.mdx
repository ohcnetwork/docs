...
A `SpecimenDefinition` is a reusable, facility-scoped template for a *kind* of specimen: what material to collect, how to prepare the patient, how to collect it, and how it's held in a container for testing. A lab maintains a repository of these, and an [Activity Definition](activity-definition.mdx) or [Service Request](service-request.mdx) references one so the same container and handling rules apply every time. When a concrete [Specimen](specimen.mdx) is instantiated from a definition, the link is kept and the specimen copies the definition's data for history and integrity.
