...
Pricing line components used by billing resources (see [Charge Item Definition](../definitions/charge-item-definition.mdx), [Charge Item](../billing/charge-item.mdx)).
...
The shapes used to *define* a value set (the FHIR `compose` structure), used by [ValueSet](../definitions/valueset.mdx).
...
- Reference: [ValueSet](../definitions/valueset.mdx) — uses `SlugBaseModel` and the ValueSet compose types
- Reference: [Charge Item Definition](../definitions/charge-item-definition.mdx) — uses `MonetaryComponent` pricing types
