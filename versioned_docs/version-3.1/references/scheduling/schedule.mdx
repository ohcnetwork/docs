...
A schedule is a repeatable block of time during which a resource — a practitioner, location, or healthcare service — is available for booking. Each schedule carries one or more **availabilities** that define how the time splits into slots, plus **exceptions** that block out specific dates.

The **Django model** (`care/emr/models/scheduling/schedule.py`) is the storage layer, including the opaque `Availability.availability` `JSONField`. The **Pydantic resource specs** (`care/emr/resources/scheduling/...`) are the API and implementation layer: they define the enums, the real shape of that JSON field, the validation rules, and the read/write schemas. Everything below the `## Models` section reflects the specs.
...
All models extend [`EMRBaseModel`](../foundation/base-model.mdx), the shared Care EMR base that provides `external_id`, `created_date`/`modified_date`, soft-delete via `deleted`, `created_by`/`updated_by`, and `history`/`meta` JSON.
...
Both charge links are managed only through the `set_charge_item_definition` action (see [Resource specs](#resource-specs-api-schema)), never through the create/update body.
...
The discriminator for the kind of entity a schedule or exception attaches to. Sent as `resource_type` on write; the matching `resource_id` is the `external_id` of that entity.
...
A bookable entity within a facility. Exactly one of `user`, `location`, or `healthcare_service` identifies the underlying resource, and `resource_type` discriminates which. It is **never created directly via the API**: `Schedule` and `AvailabilityException` writes call `get_or_create_resource(resource_type, resource_id, facility)` server-side, validating that the target belongs to the facility.
...
Uniqueness is enforced per facility and `resource_type` against each target through three `UniqueConstraint`s:
...
On read, `serialize_resource(obj)` (`resource/spec.py`) serializes a `SchedulableResource`, dispatching on `resource_type` to `UserSpec` / `HealthcareServiceReadSpec` / `FacilityLocationListSpec`.

### `Availability`

Defines how a schedule's time divides into bookable slots. A schedule can hold several availabilities, such as a morning block and an afternoon block.
...
Each entry is a single weekly recurrence window. The `JSONField` stores a **list** of these objects.
...
All specs extend `EMRResource` ([`base.py`](https://github.com/ohcnetwork/care/blob/develop/care/emr/resources/base.py)), which provides `serialize()` (DB → pydantic) and `de_serialize()` (pydantic → DB) plus the `perform_extra_serialization` / `perform_extra_deserialization` hooks. `__exclude__` lists the fields skipped during (de)serialization.
...
- **Resource resolution.** On schedule or exception create, `get_or_create_resource(resource_type, resource_id, facility)` validates that the target is in the facility (practitioner must be a `FacilityOrganizationUser`; location and healthcare-service must belong to the facility) and gets-or-creates the `SchedulableResource`. `facility` is injected from the URL via `clean_create_data`.
...
- **Locking + slot guards.** Update and destroy take a `Lock("booking:resource:<id>")`. Deleting a schedule or availability is rejected if future allocated `TokenSlot`s exist; otherwise availabilities and slots are soft-deleted (`deleted=True`).
...
- `Availability.slot_type` decides whether `slot_size_in_minutes` / `tokens_per_slot` survive: only `appointment` keeps them, while `open` and `closed` null them out (`validate_for_slot_type`).
...
- Slots are generated from `Availability` at read time, not stored as rows on the schedule.

## API integration notes

- Schedules, availabilities, and exceptions are exposed through Care's REST API; slots are generated from `Availability` rather than stored as rows.
- A `SchedulableResource` is the join point for booking. Clients send `resource_type` + `resource_id`; resolve the underlying `user`, `location`, or `healthcare_service` from `resource_type` rather than assuming one is set.
- `ChargeItemDefinition` links use `PROTECT`: a definition referenced by a schedule cannot be deleted while the schedule exists.
- `availability` on `Availability` is a JSON **list** of weekly recurrence windows (`AvailabilityDateTimeSpec`); treat it as the canonical source for the days and times the block covers.
- `create_tokens` and `tokens_per_slot` drive token and queue behaviour for high-volume, precision-optional workflows. `appointment` slot types handle time-precise booking and must divide each window evenly (≤ `MAX_SLOTS_PER_AVAILABILITY`, default 30).
