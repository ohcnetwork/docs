...
| Field definitions | `Token` fields |
...
- `status` only accepts `TokenStatusOptions` values via the update spec; `TokenSubQueue.status` only accepts `TokenSubQueueStatusOptions`.
