...
A `Token` is a numbered ticket a patient holds in a walk-in queue — use it when scheduling runs on first-come flow rather than fixed appointment slots. The patient gets a number against a [schedulable resource](../scheduling/schedule.mdx) (practitioner, location, or healthcare service), optionally routed to a sub-queue or room.
...
`TokenQueue` and `TokenSubQueue` both carry a `resource` FK to `emr.SchedulableResource`, a polymorphic binding to a practitioner (`user`), `location`, or `healthcare_service`, distinguished by `resource_type`. See [Schedule](../scheduling/schedule.mdx).
...
| `booking` | `FK → emr.TokenBooking` (CASCADE) | optional | Nullable. Links the token to a [booking](../scheduling/booking.mdx). `related_name="booking_token"` |
...
- `booking` links a token back to a [`TokenBooking`](../scheduling/booking.mdx); on retrieve it expands to the booking and its associated [encounter](../clinical/encounter.mdx).
