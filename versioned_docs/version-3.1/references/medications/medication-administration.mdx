...
`MedicationAdministration` records that a medication was (or was attempted to be) given to a patient, the actual administration event that fulfils a [`MedicationRequest`](./medication-request.mdx).
...
| Field definitions | `MedicationAdministration` fields |
| Allowed values | Enum values |
| Structured payloads | Nested specs (JSON-field shapes) |
...
`medication`, `dosage`, `status_reason`, and `performer` are stored inline as `JSONField`s rather than as separate tables, mirroring the FHIR `MedicationAdministration` structure. Use the nested spec tables above when writing these fields.
