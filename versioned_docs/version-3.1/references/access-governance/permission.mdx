...
A permission is the ability to perform one action in one context — read it as "Action on Resource", like "Can Create Patient" in the `PATIENT` context. Permissions are the atoms of Care's access control: code declares them, the sync command writes them to the database, [roles](../access-governance/role.mdx) group them, and [permission associations](../access-governance/permission-association.mdx) bind them to resources. You rarely touch this table directly; you reference its rows by `slug` when building roles.
...
- `get_enum() → Enum` — a dynamically built `str` enum of all permission slugs, used by [`RoleCreateSpec`](../access-governance/role.mdx) to constrain the `permissions` write field to known slugs.
...
Permissions reach users only through roles. `RolePermission` is the join table (`role` FK, `permission` FK, `temp_deleted`) connecting a [`RoleModel`](../access-governance/role.mdx) to a `PermissionModel`. A user's effective access is the union of permissions across their role bindings, filtered to active grants (`rolepermission__temp_deleted=False`).
...
No `PermissionCreateSpec` or `PermissionUpdateSpec` exists: permissions are reference data, never client-writable. Write specs exist for roles instead — see [Role](../access-governance/role.mdx).
...
- You can't grant a permission directly to a user. A [role](../access-governance/role.mdx) collects permissions (via `RolePermission`), and a [permission association](../access-governance/permission-association.mdx) binds that role to a resource — organization, patient, encounter, and so on. Effective access is the union of active permissions across a user's role bindings.
...
- When you write a role, `PermissionController.get_enum()` constrains the `permissions` field to known slugs; an unknown slug fails validation. See [Role](../access-governance/role.mdx).
