...
`PaymentReconciliation` records a payment made against an `Account` — and, optionally, against a specific `Invoice`. It captures how, when, by whom, and how much was paid, mirroring the FHIR [`PaymentReconciliation`](https://build.fhir.org/paymentreconciliation.html) resource.

The Django **model** is the storage layer: it stores several constrained fields as plain `CharField`s and holds `extensions` as an opaque `JSONField`. The real shape — enum values, amount computation, validation, and the read/write API schemas — lives in the Pydantic **resource specs** (`care/emr/resources/payment_reconciliation/spec.py`), built on `EMRResource`. This page documents both layers.
...
The model extends `EMRBaseModel`, the shared Care EMR base that supplies `external_id`, audit fields, and soft-delete semantics — see [Base model](../foundation/base-model.mdx).
...
These string columns map to fixed enums defined in the resource spec. The model stores them as free-form `CharField(100)`; the enum constraint is enforced at the schema/API layer, not by the database. See [enum values](#enum-values) for the full lists.
...
- **Amount validator** (`model_validator(mode="after")`, `check_amount_or_factor`): raises `"Returned amount cannot be greater than tendered amount"` when `returned_amount >= tendered_amount`, then sets `amount = tendered_amount - returned_amount`. The write spec's `amount` field is therefore advisory — it is always recomputed.
...
Every foreign key uses `on_delete=PROTECT`, so a payment record blocks deletion of the facility, invoice, account, or location it references.
...
- **FK resolution** happens in `perform_extra_deserialization`: clients pass `external_id` UUIDs for `account`, `target_invoice`, and `location`, and these resolve to instances at de-serialization time.
...
- Exposed through Care's REST API. Write requests use `PaymentReconciliationWriteSpec` for both create and update; list responses use `PaymentReconciliationMinimalReadSpec`, and detail/retrieve responses use `PaymentReconciliationReadSpec` / `PaymentReconciliationRetrieveSpec`.
...
- Classification fields (`reconciliation_type`, `status`, `kind`, `issuer_type`, `outcome`, `method`) are constrained to the fixed enums above by the Pydantic schema — the API rejects invalid values even though the column is a plain `CharField`.
