...
A charge item definition is the billing template that answers "how much does resource X cost". It binds a billable resource to price components such as base rate, surcharges, discounts, and taxes, which are evaluated when a [charge item](../billing/charge-item.mdx) is generated during data entry.
...
| Field definitions | `ChargeItemDefinition` fields |
