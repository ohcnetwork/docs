...
A charge item tracks the financial cost of a service or product supplied to a patient. It records what was charged, the quantity, the unit and total price components, and any applied discounts or override reasons. Every charge item belongs to an [`Account`](./account.mdx) and is created against a patient (optionally an encounter).
...
| Field definitions | `ChargeItem` fields |
...
`ChargeItem` extends `EMRBaseModel`, the shared Care EMR base with `external_id`, audit fields (`created_by`, `updated_by`, `created_date`, `modified_date`), and soft-delete semantics. See [Base model](../foundation/base-model.mdx).
