...
A supply delivery records the movement of a product from one location — or an external supplier — to a destination location. Together with supply dispensing, deliveries are the source of truth for facility inventory: `supply delivered − supply dispensed = current stock`. Deliveries are grouped under a `DeliveryOrder`, which is typically created from a [supply request](./supply-request.mdx).

The Django model is the **storage layer**: `status`, `delivery_type`, and `supplied_item_condition` are plain `CharField`s, and `extensions` is an opaque `JSONField`. The **real shape** of those fields — enum values, validation, and the read/write schemas — lives in the Pydantic resource specs documented below.
...
Both models extend [`EMRBaseModel`](../foundation/base-model.mdx), the shared Care EMR base providing `external_id`, audit fields `created_by`/`updated_by`, `created_date`/`modified_date`, soft-delete via `deleted`, and `history`/`meta` JSON.
...
`patient` and `patient_invoice` use `PROTECT`, so a referenced patient or invoice cannot be deleted while a delivery order points to it.
