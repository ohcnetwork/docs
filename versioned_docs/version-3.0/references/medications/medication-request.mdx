...
| Field definitions | `MedicationRequest` fields |
...
Each entry of the `dosage_instruction` JSON list deserializes to a `DosageInstruction` (Pydantic, snake_case keys). Use the structure below when writing dosage instructions.
...
### Validation for create (`MedicationRequestSpec`)
