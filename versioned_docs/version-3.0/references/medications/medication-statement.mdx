...
The Django model handles **storage**: `medication` and `effective_period` are opaque `JSONField`s whose real structure lives in the Pydantic **resource specs** (`care/emr/resources/medication/statement/`). The specs define the enums, the nested JSON shapes, validation, the value-set binding on `medication`, and the read/write API schemas. This page documents both.
...
The **Type** column gives the Django storage type; the **Spec shape** column gives the structured shape the Pydantic resource spec enforces when reading and writing over the API.
...
Both foreign keys cascade on delete, so removing a patient or encounter also removes the associated medication statements.
...
> The `care/emr/resources/medication/valueset/` directory holds additional value sets used by other medication resources (route, body site, administration method, additional instruction, as-needed reason, medication-not-given reason). `MedicationStatement` itself binds only `medication` → `system-medication`.
...
All specs subclass `EMRResource` (`care/emr/resources/base.py`), which provides `serialize` (DB object → pydantic) and `de_serialize` (pydantic → DB object). `__model__ = MedicationStatement`; `__exclude__ = ["patient", "encounter"]` keeps those fields out of the generic field copy so the extra hooks handle them instead.
...
- **Update is restricted** — `MedicationStatementUpdateSpec` accepts only `status`, `effective_period`, and `note`; `medication`, `reason`, `dosage_text`, `information_source`, and `encounter` are not updatable through it.
...
This resource has **no** `status_history` — status is a single current value, not an appended log.
...
- Updates go through `MedicationStatementUpdateSpec` and can change only `status`, `effective_period`, and `note`.
