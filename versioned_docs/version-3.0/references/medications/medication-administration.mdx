...
`MedicationAdministration` records that a medication was given to a patient — or that giving it was attempted — fulfilling a [`MedicationRequest`](../medications/medication-request.mdx). You write one each time a dose is administered against an order. The Django model is the storage layer; several columns are opaque `JSONField`s whose structure comes from the Pydantic resource specs in `care/emr/resources/medication/administration/`. Both layers are documented below.
...
| Model | Purpose |
| --- | --- |
| `MedicationAdministration` | The administration event that fulfils a [`MedicationRequest`](../medications/medication-request.mdx) |

`MedicationAdministration` extends `EMRBaseModel`, the shared Care EMR base that provides `external_id`, audit fields, and soft-delete semantics (see [Base model](../foundation/base-model.mdx)). It maps to the FHIR [`MedicationAdministration`](https://hl7.org/fhir/medicationadministration.html) resource.
...
| Field | Type | Notes |
| --- | --- | --- |
| `actor` | `UUID4` (required) | The user who performed the administration. Validated server-side to reference an existing [`User`](../access-governance/user.mdx) (`field_validator` raises "User not found") |
