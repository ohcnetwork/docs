...
`MedicationAdministration` records that a medication was given — or that giving it was attempted — to a patient. It is the administration event that fulfils a [`MedicationRequest`](./medication-request.mdx). The Django model is the storage layer; several of its fields are opaque `JSONField`s whose real structure comes from the Pydantic **resource specs** in `care/emr/resources/medication/administration/`. This page documents both layers.
...
`MedicationAdministration` extends `EMRBaseModel`, the shared Care EMR base that provides `external_id`, audit fields, and soft-delete semantics (see [Base model](../foundation/base-model.mdx)). It maps to the FHIR [`MedicationAdministration`](https://hl7.org/fhir/medicationadministration.html) resource and closes the loop on the prescriber's intent captured in a `MedicationRequest`.

## `MedicationAdministration` fields

The types below give the **storage** column type and, where the column is a `JSONField`, the **structured shape** enforced by the resource spec.
...
Structured shape of the `dosage` `JSONField`. All fields are optional.
...
The following medication value sets are defined alongside these but are not referenced by this resource's specs: Medication Not Given Reason (`system-medication-not-given`, `is-a 242990004` + `is-a 182895007`), Additional Instruction (`system-additional-instruction`, `is-a 419492006`), As Needed (`system-as-needed-reason`, `is-a 404684003`).
...
`medication`, `dosage`, `status_reason`, and `performer` are stored inline as `JSONField`s rather than in separate tables, mirroring the FHIR `MedicationAdministration` structure; their real shape comes from the resource specs above.

## Methods & save behaviour

- `EMRResource.serialize(obj)` builds a read schema from a DB row, copying mapped fields and then calling `perform_extra_serialization` (sets `id`, flattens FK references, serializes nested `administered_product`, adds audit users).
- `EMRResource.de_serialize(obj?)` builds or updates a DB row from a write schema using `model_dump(exclude_defaults=True)`, then calls `perform_extra_deserialization`. On create it resolves and assigns `encounter`, `patient`, `request`, and `administered_product`.
...
- Set either `medication` or `administered_product`, never both. `administered_product` is the bridge to inventory — set it when a specific catalogued product was used, so the administration can be reconciled against supply.
