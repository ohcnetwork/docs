...
An `Account` is a financial bucket that aggregates every transaction made against a patient within a facility. The Django model is the **storage layer** — several fields are opaque `JSONField`s whose real structure lives in the Pydantic **resource specs** (`care/emr/resources/account/`). This doc enriches the model fields with the spec-defined enums, JSON shapes, validation, and API read/write schemas.
...
An account is perpetual by default. Charge items are placed on the patient's default account within a facility, and initially only one active+open account exists per patient per facility (see [`get_default_account`](#default-account)). Once a patient is discharged, accounts can be balanced and closed. Invoices are created against accounts or against the items within them.
...
- If both are present, `start <= end` (`"Start Date cannot be greater than End Date"`).
...
Key `MonetaryComponent` validation: `tax_included_amount` is only allowed on a `base` component; a `base` component must have an `amount`, no `factor`, and no `conditions`; `amount` and `factor` are mutually exclusive; exactly one of `amount`/`factor` is required unless it is a `global_component` with a `code`.
...
- **Create** (`AccountCreateSpec`): mixes in `ExtensionValidator` (which validates `extensions`) and requires `patient` as a UUID. `perform_extra_deserialization` resolves it via `get_object_or_404(Patient, external_id=...)` and assigns `obj.patient`. `patient` is otherwise excluded from the base serialize loop.
...
- **Status / billing_status** are validated against `AccountStatusOptions` / `AccountBillingStatusOptions`; the Pydantic enums reject invalid values.
- **Read serialization**: `perform_extra_serialization` sets `mapping["id"] = obj.external_id`. `AccountReadSpec` also serializes the patient (`PatientListSpec`) and renders tags via `SingleFacilityTagManager`; `AccountRetrieveSpec` serializes the full patient (`PatientRetrieveSpec`, scoped to `obj.facility`) and, when present, the primary encounter (`EncounterRetrieveSpec`).
- **Totals are read-only**: balance/aggregate fields appear only on read specs. `sync_account_items` recomputes them; they are never set from client payloads.
...
This materializes the default account when the first charge item is added — clients normally do not create accounts directly.
...
All sums use `care_round`. `total_net`, `cached_items`, and `total_price_components` are currently **not** updated (their logic is commented out), so they stay at their defaults. `rebalance_account_task(account_id)` is the Celery task wrapper that runs the sync and saves.

## Related models

`Account` is the only class defined in `account.py`. Its relationships are:
...
[Charge items](./charge-item.mdx) reference the account they belong to and drive its totals; [invoices](./invoice.mdx) and [payment reconciliations](./payment-reconciliation.mdx) are created against the account or its items.
