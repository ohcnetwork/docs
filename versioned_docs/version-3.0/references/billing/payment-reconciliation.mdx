...
`PaymentReconciliation` records a payment made against an `Account` and, when available, a specific `Invoice`. It captures how, when, by whom, and how much was paid, mirroring the FHIR [`PaymentReconciliation`](https://build.fhir.org/paymentreconciliation.html) resource.
...
| Field definitions | `PaymentReconciliation` fields |
