...
| Field definitions | `PermissionModel` fields |
...
| --- | --- | --- | --- | --- |
| `slug` | `CharField(1024)` | yes | - | `unique`, indexed. Stable machine identifier for the permission (the `enum` member name, e.g. `can_create_patient`). Code, specs, and the API reference a permission by `slug`, not `external_id`. On the API it is the `lookup_field` |
