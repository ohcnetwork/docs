...
- `PlugConfig` extends plain `django.db.models.Model`, with no audit or soft-delete fields.

## `User` fields

`User` extends `AbstractUser`, inheriting `password`, `email`, `first_name`, `last_name`, `is_staff`, `is_active`, `is_superuser`, `last_login`, and `date_joined` from Django. The fields below are Care additions or overrides. Queries go through `CustomUserManager`, which filters out `deleted=True` rows by default (`get_entire_queryset()` bypasses the filter).
...
Referenced by the resource layer (defined under `care/emr/resources/base.py` and `care/emr/resources/common/`). User specs bind no coded fields, but the base period/contact types are part of the shared vocabulary:
...
`User` overrides neither `save()` nor `delete()`, but it exposes several notable helpers:
...
- `deleted` is honoured by `CustomUserManager`: soft-deleted users are excluded from normal queries, but cached `UserSpec` lookups use the base manager (`use_base_manager=True`) and can still resolve deleted users (e.g. as `created_by`).
- Authorization is not stored on `User`. Access resolves through the roles/permissions/organization system, and `cached_role_orgs` is a platform-maintained cache — do not write it directly from clients. New users supply `role_orgs` (organization + role UUIDs) at create time.
