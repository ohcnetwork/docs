...
| Field definitions | `Consent` fields |
...
The table lists the persisted field and the API type accepted or returned by the resource specs.
...
| `period` | `JSONField` (default `dict`) | `PeriodSpec { start, end }` | Validity window. Both ends tz-aware; `start ≤ end`; `start ≥ date` is enforced on create. See [PeriodSpec](#periodspec-nested-shape) |
