...
| Field definitions | `NoteThread` fields |
...
| Field | Type | Notes |
| --- | --- | --- |
| `title` | `CharField(255)` | Nullable and blank-allowed in the database, the write spec requires it (`title: str`, `max_length=255`) |
...
| `message_history` | `JSONField` | Defaults to `{}`. The update spec writes the shape below and maintains it server-side on edit. Exposed read-only |
...
- `title` is **required** on write (`Field(..., max_length=255)`), even though the column is nullable.
