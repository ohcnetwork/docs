...
- `convert_od_to_observation` (`observation.py`) builds an [`Observation`](../clinical/observation.mdx) from a definition for a given encounter, carrying over `category` and `code` and defaulting `status` to `final`.
