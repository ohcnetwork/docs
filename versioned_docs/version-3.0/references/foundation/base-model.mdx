...
Technical reference for the shared base layer that every Care EMR resource builds on. It has **two layers**:
...
- **API / implementation layer** — Pydantic resource specs built on `EMRResource` (`care/emr/resources/base.py`). They define enums, field validation, the read/write schemas, and the structured shape of those JSON fields (via nested specs). The shared structured types live in `care/emr/resources/common/` and are reused across nearly every resource.

This page documents both layers and the shared common spec types. Concrete resources (for example [Patient](../clinical/patient)) inherit a storage base and define their own resource specs on top of `EMRResource`.
...
All five storage models are `abstract = True`: none defines a database table of its own. A concrete resource model inherits one of them and contributes the columns described below to its own table.
...
The API specs form a parallel hierarchy rooted at `pydantic.BaseModel`:
...
`objects` is overridden to a [`BaseManager`](#basemanager) instance, so the default queryset returns only live rows.
...
The standard base for EMR resource models. It extends `BaseModel`, so it also carries `external_id`, the timestamps, and `deleted`, plus the following:
...
The default manager applied as `objects` on `BaseModel`. It filters out soft-deleted rows at the queryset level:
...
Abstract base for per-entity feature-flag tables (for example facility or organization flags). It extends `BaseModel`, stores a single validated flag name, and serves all lookups from the cache.
...
**Spec naming convention.** Each concrete resource exposes a small set of specs built on `EMRResource`. Read them as one table per resource:
...
Coded fields commonly bind to a **value set** (a `system`/`code` allow-list). The resource spec declares the binding (e.g. via `json_schema_extra={"slug": "<valueset-slug>"}` on the field) and validates the submitted `Coding` against it in `perform_extra_deserialization`. Status fields commonly maintain a server-side `status_history` appended on create/update. See each resource page for its exact bindings and side effects.
