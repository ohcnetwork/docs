...
`org_type` is a `CharField` in the DB, but every write binds to `OrganizationTypeChoices` (`resources/organization/spec.py`).
...
`FacilityOrganization` in practice uses the `team` and `root` types; the spec binds the same `OrganizationTypeChoices` enum (instance scope). `govt`/`role` orgs and root orgs are restricted to superadmin management.
