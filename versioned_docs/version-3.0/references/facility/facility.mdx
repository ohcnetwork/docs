...
On creation, `save()` provisions a root [`FacilityOrganization`](../access-governance/organization.mdx) named `Administration` (`org_type="root"`, `system_generated=True`) and a `FacilityOrganizationUser` linking `created_by` to it with the `FACILITY_ADMIN_ROLE`. Feature flags live in [`FacilityFlag`](facility-flag.mdx) and surface via `get_facility_flags()`.
