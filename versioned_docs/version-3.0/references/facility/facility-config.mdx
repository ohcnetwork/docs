...
Technical reference for the per-facility monetary/billing configuration in Care EMR.
...
| Field definitions | `FacilityMonetoryConfig` fields |
...
- Write `discount_codes`, `discount_monetary_components`, and `discount_configuration` through `FacilityMonetaryCodeSpec` and the nested `Coding` / `MonetaryComponentDefinition` / `DiscountConfiguration` specs so validation catches invalid codes, components, and stacking rules.
