...
A `HealthcareService` describes a service offered at a facility (e.g. cardiology, lab, radiology, pharmacy), scoped to one or more locations and a managing facility organization. The Django model is the **storage** layer: `service_type` and `styling_metadata` are opaque `JSONField`s whose real structure lives in the Pydantic **resource specs**. Those specs also define the bound value set, the enum for `internal_type`, and the read/write API schemas.
...
| `extra_details` | `TextField` | Free-text additional details. Not nullable and has no DB default, so a value must be supplied (an empty string is allowed). Required in write specs |

### Scope & placement

| Field | Type | Notes |
| --- | --- | --- |
| `facility` | `FK → facility.Facility` | `PROTECT`; nullable (`default=None`, `blank=True`). The facility offering the service. Excluded from all specs (`__exclude__ = ["facility"]`) and set server-side from the URL context, never via the request body |
| `locations` | `ArrayField[int]` | Defaults to `[]`; a list of `FacilityLocation` **primary-key integers** (not FK rows) where the service is provided. Write specs accept location `external_id`s (UUIDs); retrieve specs return serialized location objects |
...
| Field | Type | Notes |
| --- | --- | --- |
| `styling_metadata` | `JSONField` → `dict` | Nullable in DB; defaults to `{}` in the specs. An open JSON bag of UI presentation hints (e.g. colors, icons), with no backend semantics |

## Coded field shapes (JSON fields)

### `service_type` → `Coding`

`service_type` stores a single `Coding` object. In the API it is a `ValueSetBoundCoding` whose `code` is validated against the **Healthcare Service Type Code** value set during de-serialization. Optional (`None`).
...
Registered via `register_valueset(...)` and `register_as_system()`. The `service_type.code` submitted on create or update must resolve within this value set.

## Enums

### `HealthcareServiceInternalType` values

Constrains `internal_type` in the specs (`str` enum). The model column is a plain `CharField`, so legacy or other values may exist in storage.
...
The facility at which the service is offered. `PROTECT` prevents deleting a facility that still has healthcare services. Excluded from specs and set from the request context. See [Facility](./facility.mdx).
...
`locations` is an `ArrayField` of integer PKs rather than a relational join. Each entry references a `FacilityLocation` row by primary key, and application code handles resolution and validation instead of a database foreign key. Write specs accept location `external_id`s (UUIDs); the retrieve spec embeds each location, serialized via `FacilityLocationListSpec`. See [Location](./location.mdx).

## Resource specs (API schema)

All specs extend `EMRResource` (`care/emr/resources/base.py`), which provides `serialize` (DB object → spec) and `de_serialize` (spec → DB object), along with the `perform_extra_serialization` / `perform_extra_deserialization` hooks. `__exclude__ = ["facility"]` keeps `facility` out of every payload.
...
- **`service_type`** — coded as a `ValueSetBoundCoding`. During de-serialization, the `code` is validated against the `healthcare-service-type-code` value set (`validate_valueset`). `extra="forbid"` on the underlying `Coding` rejects unknown keys.
- **`managing_organization`** (write) — `perform_extra_deserialization` resolves the supplied UUID via `get_object_or_404(FacilityOrganization, external_id=...)` and assigns the FK; if the value is falsy, it sets `managing_organization = None`.
...
- **`facility`** — excluded from all specs and assigned server-side, so it cannot be set or changed through the API body.
- This resource has no `status` field, so there is no server-maintained `status_history`.
...
- On the API boundary, `internal_type` is one of `pharmacy`, `lab`, `scheduling`, or `store`.
- On write, `locations` is a list of location `external_id`s (UUIDs); detail responses embed full location objects. No DB-level integrity check confirms that referenced locations belong to the same `facility`.
- On write, `managing_organization` is the org `external_id`; detail responses embed the serialized organization.
