...
`MetaArtifact` is a generic, polymorphic metadata record attached to any Care object by type and external ID. The Django model handles storage: `object_value` is an opaque `JSONField`, and `associating_type` / `object_type` are bare `CharField`s. The constrained values and the API request/response schemas live in the Pydantic resource specs.
...
A composite database index covers these two fields (see [Methods & save behaviour](#methods--save-behaviour)), so lookups by associated object stay efficient.
...
These choice classes constrain `associating_type` and `object_type` at the API layer. They are defined in `spec.py` (there is no separate `constants.py` / `valueset.py` for this resource), and both are `str` enums.
...
- **Update**: `MetaArtifactUpdateSpec` exposes only `object_value` and `note` (plus the inherited optional `id`). It does not re-accept `associating_type`, `associating_id`, `object_type`, or `name`, so association and classification are immutable after creation.

No coded field on this resource binds to a value set (there is no `valueset.py`); the only constrained vocabularies are the two `str` enums above.
...
- The generic `associating_type` + `associating_external_id` pair lets a single table store metadata for many different Care object types without per-type foreign keys. Clients reference the target by its `external_id` (UUID), never its internal primary key, and send it as `associating_id` on create.
- `associating_type` and `object_type` are open `CharField`s in storage but are constrained at the API by `str` enums (`patient` / `encounter` and `drawing` respectively). Adding new association or artifact kinds means extending those enums in `spec.py`, not running a schema migration.
