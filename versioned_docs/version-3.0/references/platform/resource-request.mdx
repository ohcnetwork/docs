...
Both models extend [`EMRBaseModel`](../foundation/base-model.mdx), the shared Care EMR base that adds `external_id`, audit fields (`created_by`, `updated_by`, `created_date`, `modified_date`), and soft-delete semantics.
...
All specs build on `EMRResource` ([`base.py`](https://github.com/ohcnetwork/care/blob/develop/care/emr/resources/base.py)), which provides `serialize` (DB object → pydantic, the read path) and `de_serialize` (pydantic → DB object, the write path), along with the `perform_extra_serialization` / `perform_extra_deserialization` hooks. `__exclude__` lists the fields the base (de)serializer skips so they can be handled explicitly (for example, FK resolution).
...
- `validate_assigned_to_user` (model validator, `mode="after"`): when `assigned_to` is set, `assigned_facility` is **required**, and the assigned user must be a member of the assigned facility (checked via `FacilityOrganizationUser`). Otherwise it raises `ValueError`.
...
A comment entry on a resource request. Deleting a comment does not cascade to the request, because the FK uses `PROTECT`.
...
- `status` and `category` are open `CharField`s in the database but are constrained to `StatusChoices` / `CategoryChoices` by the spec, so only those values pass API validation.
- `priority` is a required integer at the API layer even though the model column is nullable.
- Foreign keys are sent and returned as `external_id` UUIDs in write payloads and as nested objects (`FacilityReadSpec`, `PatientListSpec`, `UserSpec`) in read payloads.
...
- `related_patient` is optional and cascades on patient deletion. `origin_facility` is `PROTECT`-ed: it cannot be deleted while requests reference it, and it cannot be changed after the request is created.
