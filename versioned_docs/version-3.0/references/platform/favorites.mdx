...
| Field definitions | `UserResourceFavorites` fields |
...
There is no read schema for the favorited objects themselves: favorited resources are returned through the **owning resource's own list endpoint** (e.g. the questionnaire list serializer), filtered and ordered by `FavoritesFilter`. The `favorite_lists` action returns only `{"lists": [<list name>, ...]}`.
