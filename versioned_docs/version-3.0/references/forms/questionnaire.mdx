...
Questionnaires (a.k.a. **forms**) are generalized, FHIR-inspired data-collection structures: a versioned tree of questions answered about a subject (a patient or an encounter). They power both clinical data capture (answers become [Observations](../clinical/observation.mdx)) and non-clinical structured data.
...
| Field definitions | `Questionnaire` fields |
...
`Questionnaire.questions` is written as a recursive list of `Question` (`QuestionnaireBaseSpec`). The table below is the API contract, including the field types and validation rules:
...
`QuestionnaireResponseTemplate.template_data` is validated as:
...
- `questions` must follow the recursive [`Question`](#question-nested-shape) spec on the API (unique `link_id`/`id`, choice/quantity/group constraints, value-set existence). `styling_metadata` remains free-form.
