...
A `QuestionnaireResponse` is the set of answers a patient gives to a [questionnaire](./questionnaire.mdx); a `FormSubmission` is the envelope that carries that payload through a draft-to-finalised lifecycle. You touch these when reading back what was answered or when submitting new answers.
