...
Technical reference for the `QuestionnaireResponse` and `FormSubmission` modules in Care EMR — the **submitted answers** to a [questionnaire](./questionnaire.mdx) for a patient, plus the submission envelope that carries a draft or finalised payload.

The Django models are the **storage** layer. The Pydantic **resource specs** define the submit payload, the answer structure, the status enums, and the read/write API schemas.
...
Joins each entry in `responses` with its matching question definition from `questionnaire.get_questions_by_id()`, producing a list of `{ "answer": <response>, "question": <question> }` — a human-readable view of the answers against the current questionnaire. Returns `[]` when there are no responses or no linked questionnaire.
...
New responses are created through the questionnaire submit flow (`QuestionnaireSubmitRequest`), not a generic create spec.
...
- `FormSubmission` supports a draft→submitted lifecycle. Its `response_dump` is an opaque JSON payload validated by the questionnaire definition, not by the model.
