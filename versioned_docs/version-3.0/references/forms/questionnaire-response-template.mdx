...
A `QuestionnaireResponseTemplate` is a clinician's saved "favourite" or order set: a reusable, shareable bundle that pre-fills a [questionnaire response](./questionnaire-response.mdx) along with its medication requests and activity definitions. Reach for it when building authoring shortcuts that seed several clinical resources from one named template.
