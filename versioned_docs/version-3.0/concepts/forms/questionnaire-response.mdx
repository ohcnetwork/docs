...
A **questionnaire response** is the set of answers a clinician or staff member records when they fill in a [questionnaire](./questionnaire.mdx) for a patient. It turns a blank form — a triage screen, a vitals chart, an intake assessment — into a permanent, attributed piece of that patient's clinical record.
...
- **Questionnaire** — the response points back to the [questionnaire](./questionnaire.mdx) it answered; rendering the answers joins them against that definition so labels stay human-readable
...
Questionnaire responses are governed by the **patient** permission set — there is no separate response permission. Two permissions apply directly.

| Permission | Description | System Roles |
| --- | --- | --- |
| `can_submit_patient_questionnaire` | Submit a questionnaire about a patient, creating a new response | Volunteer, Staff, Doctor, Nurse, Admin, Facility Admin, Administrator |
| `can_view_questionnaire_responses` | View the questionnaire responses recorded on a patient | Volunteer, Staff, Doctor, Nurse, Admin, Facility Admin, Administrator, Pharmacist |

Roles are granted through a user's organization, facility, or patient memberships, and permissions cascade down the organization tree — so access to a patient's responses follows from access to the patient.

## Related

- Reference: [Questionnaire Response (technical)](../../references/forms/questionnaire-response.mdx)
- Concept: [Questionnaire](./questionnaire.mdx) — the form definition being answered
- Concept: [Questionnaire Response Template](./questionnaire-response-template.mdx) — reusable pre-fill templates
