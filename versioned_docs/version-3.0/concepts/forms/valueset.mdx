...
Value sets are shared terminology, so any authenticated user can read them — they power the code pickers everyone relies on while documenting care. Authoring and editing are administrator-gated, and system-defined value sets are not editable by deployments at all.
