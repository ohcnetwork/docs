...
The key distinction is that a charge item definition is the *template*, not the bill. It describes what something should cost in general; the actual money owed for a specific patient on a specific day lives on a [charge item](../billing/charge-item.mdx). Think of the definition as the menu price and the charge item as the line on the receipt.
...
- **Charge item** — when a service is delivered, the definition is applied to create a [charge item](../billing/charge-item.mdx), copying its pricing into an actual chargeable line
- **Account & invoice** — those charge items later accumulate on an [account](../billing/account.mdx) and are collected onto an [invoice](../billing/invoice.mdx) for payment
...
- Reference: [Charge Item Definition (technical)](../../references/definitions/charge-item-definition.mdx)
