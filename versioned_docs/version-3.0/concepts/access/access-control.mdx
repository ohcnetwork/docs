...
Care governs what each person can see and do through **role-based access control (RBAC)**. A **permission** is the right to perform one action (for example, "Can Create Patient"); a **role** is a named bundle of permissions; and a role is granted to a user *within a boundary* — a facility, an organization, or a single patient. Care never assigns access to a user directly: it always flows permission → role → membership.

## What it represents

The access-control model combines three pieces that work together:
...
- **Roles** — flat, named collections of permissions (for example, *Doctor* or *Nurse*). A role is not a job title or a type; it is simply "this set of permissions." Care ships with built-in roles, and deployments can add their own.
...
Care seeds a set of built-in (system) roles. You cannot edit or delete these roles, and each applies within the boundaries shown below.
...
Care records memberships by boundary type:
...
Organizations form a **tree**: a government organization can contain sub-organizations, and a facility can contain departments. A role granted at a parent **cascades to every descendant** — if you are an Administrator of a district, you hold that access in all facilities and units beneath it, without a membership at each one. To check access, Care looks at the target plus all of its ancestors and collects every permission you hold along that chain.

## How a permission is decided

Each permission carries a **permission context** describing the resource it governs. When Care serializes a resource for a user, it counts only the permissions in the relevant contexts:
...
So a request to read a patient becomes "which roles do I hold that reach this patient, and do any of them carry a `PATIENT`/`FACILITY` permission for this action?" Care computes the answer live from the user's memberships and the organization tree.
...
Roles reach users through organization, facility-organization, or patient memberships, and the organization tree scopes that access: a permission held at a parent organization cascades to its descendants, and a user's effective permissions in any context are the union of all roles that reach it. The permissions themselves are platform-maintained reference data — administrators assign roles, not individual permissions.
...
Care's roles and permissions form an internal authorization model rather than a clinical resource, so they do not map to a single FHIR resource. They align in spirit with FHIR's access-control concepts (such as the `Permission` and `Role`/`PractitionerRole` resources), but enforcement is Care-specific.
