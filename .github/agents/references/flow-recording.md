...
```sh
grep -h "^REACT_CARE_API_URL" ~/ohc.network/care_fe/.env.local ~/ohc.network/care_fe/.env
```
...
4. **Fresh auth state** — in `care_fe`, run `npx playwright test --project=setup`.
   ...
   session. The `care_docs` config reads these files by relative path.
   Warning: this step restores the local database. Confirm with the user first.

## Recording config

```ts
// care_docs/playwright.config.ts
use: {
...
`care_docs/Clinical/Encounter/flows/create_an_encounter.md`.
